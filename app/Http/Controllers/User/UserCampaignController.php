...
class UserCampaignController extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($campaigns as $campaign) {
            $lastDisbursement = CampaignDisbursement::where('campaign_id', $campaign->campaign_id)
                ->latest()
                ->first();

            $hasRefund = DB::table('tb_donation_refunds')
            ->join('tb_donations', 'tb_donation_refunds.donation_id', '=', 'tb_donations.donation_id')
    ...
    {
        $hasRequest = CampaignDisbursement::where('campaign_id', $campaign->campaign_id)->exists() ||
    ...
    {
        $campaign = Campaign::where('campaign_id', $id)->where('user_id', Auth::id())->firstOrFail();

        if ($campaign->current_amount < $campaign->target_amount && !$campaign->is_deadline_passed) {
    ...
    {
        $existingDisbursement = \App\Models\CampaignDisbursement::where('campaign_id', $id)->exists();
        ...
        if (!$campaign->is_deadline_passed || $campaign->current_amount >= $campaign->target_amount) {
            return back()->with('error', 'Refund tidak tersedia untuk campaign ini.');
        }

        $donations = Donation::where('campaign_id', $id)->where('payment_status', 'paid')->get();
        ...
        $campaign->update(['campaign_status' => 'canceled']);
